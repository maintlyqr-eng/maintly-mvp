...
// "industrial" (ver el comment de globals.css). Dos cambios de fondo:
//   1. Mobile no tenía NINGUNA identidad visual — la imagen de fondo se
//      ocultaba por completo debajo de md: y quedaba un form flotando
//      sobre blanco liso. Ahora hay una franja "hero" arriba (recorte de
//      la misma login-side.png, centrado en el círculo QR/engranaje via
//      object-position, degradé hacia carbón abajo) para que el celular
//      también vea la identidad de marca antes de loguearse.
//   2. El form pasa a vivir dentro de una tarjeta real (fondo blanco,
//      shadow-industrial-light, línea de acento roja arriba) en vez de
//      flotar directo sobre la imagen — y usa framer-motion para una
//      entrada suave en vez de aparecer de golpe.
...
function LoginForm() {
  ...
  return (
    <main className="relative min-h-dvh bg-cold-white overflow-x-hidden flex flex-col md:block">

      {/* ── HERO MOBILE (< md): antes esta franja no existía y el celular
          perdía toda la identidad visual del Login — la imagen de fondo se
          ocultaba entera debajo de md:. Es un recorte de la misma
          login-side.png, centrado (object-position) en el círculo QR/
          engranaje, con un degradé hacia carbón abajo para anclar con la
          tarjeta del form. ── */}
      <div className="md:hidden relative h-[190px] shrink-0 overflow-hidden bg-carbon">
        <Image
          ...
          priority
          sizes="100vw"
          style={{ objectFit: "cover", objectPosition: "35% 38%" }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-carbon" />
      </div>

      {/* ── IMAGEN DE FONDO COMPLETA (desktop, incluye el panel blanco dibujado) ── */}
      ...
      {/* ── FORMULARIO ── */}
      <div className="relative z-10 flex-1 md:min-h-dvh flex items-center justify-center px-6 py-8 md:px-0 md:py-0">
        <div className="w-full flex md:max-w-[1536px] md:aspect-[1536/1019]">
          ...
          <div className="flex items-center w-full md:w-[38%] md:pr-[5%]">
            <motion.div
              ...
              className="w-full max-w-[420px] mx-auto relative bg-white rounded-[28px] shadow-industrial-light px-7 py-8 md:px-8 md:py-9 overflow-hidden"
            >
              ...
              <div className="text-center mb-7">
                <motion.div
                  className="inline-flex items-center justify-center w-14 h-14 rounded-full border border-red-100 bg-red-50 mb-4"
                  animate={{ boxShadow: ["0 0 0 0 rgba(220,38,38,0.15)", "0 0 0 8px rgba(220,38,38,0)", "0 0 0 0 rgba(220,38,38,0)"] }}
                  transition={{ duration: 2.6, repeat: Infinity, ease: "easeOut" }}
                >
                  <User size={24} className="text-red-600" />
                </motion.div>
                <h2 className="text-[26px] font-black text-zinc-900">{t("welcomeBack")}</h2>
              ...
              <AnimatePresence>
                {error && (
                  <motion.div
                    ...
                    className="rounded-lg bg-red-50 border border-red-200 px-3 py-2 flex items-center gap-2 text-[12px] text-red-700 overflow-hidden"
                  >
              ...
              <motion.button
                ...
                className="w-full flex items-center justify-center gap-3 border border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300 hover:shadow-sm disabled:opacity-60 disabled:cursor-not-allowed transition-colors duration-150 py-[12px] rounded-xl text-[13px] font-semibold text-zinc-700"
              >
                {googleLoading ? (
