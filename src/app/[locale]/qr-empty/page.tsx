...
export default function QrEmptyPage() {
  const t = useTranslations("QrEmptyPage");

  // NOTE: the "Login to Assign this QR" / "Create an Account" buttons have
  // no onClick/href in the original English page either — pre-existing,
  // not something introduced by this i18n pass. Left exactly as-is.
  const benefits = [
    { icon: Tag, title: t("benefit1Title"), sub: t("benefit1Sub") },
    { icon: TrendingUp, title: t("benefit2Title"), sub: t("benefit2Sub") },
    { icon: ShieldCheck, title: t("benefit3Title"), sub: t("benefit3Sub") },
  ];
          ...
          >
            <div className="w-full max-w-[360px] mx-auto flex flex-col items-center px-4">
              ...
              <p className="text-center text-[10px] text-zinc-400 mb-2.5">
                {t("loginNotePrefix")} <span className="font-bold text-red-600">Maintler</span> {t("loginNoteSuffix")}
              </p>
              ...
              <a href="#" className="w-full flex items-center justify-between text-zinc-500 hover:text-zinc-700 transition-colors">
                <div className="flex items-center gap-2">
                  <Eye size={13} />
                  <span className="text-[11px]">{t("continueAsVisitor")}</span>
                </div>
                <span className="text-[10px] text-zinc-400">{t("viewQrInfoOnly")} ›</span>
              </a>
            </div>
          </div>
        </div>
