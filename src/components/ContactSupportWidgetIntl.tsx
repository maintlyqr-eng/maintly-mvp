...
// Localized twin of src/components/ContactSupportWidget.tsx — same
// reasoning as DashboardSidebarIntl.tsx. Time labels use the active
// next-intl locale instead of the original's hardcoded "es-AR".
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md h-[75vh] max-h-[560px] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-[15px] font-black text-zinc-900 leading-tight">{t("contactSupport")}</h2>
                  <p className="text-[11px] text-zinc-400 leading-tight">{t("maintlyTeam")}</p>
                </div>
              </div>
            ...
            {confirmClear && (
              <div className="flex items-center justify-between gap-2 px-5 py-2.5 bg-red-50 border-b border-red-100 shrink-0">
                <span className="text-[11px] text-red-700 font-medium">{t("clearConfirmQuestion")}</span>
                <div className="flex items-center gap-1.5 shrink-0">
                  <button onClick={handleClear} className="text-[11px] font-bold text-white bg-red-600 hover:bg-red-500 px-2.5 py-1 rounded-lg transition-colors">{t("confirm")}</button>
                  <button onClick={() => setConfirmClear(false)} className="text-[11px] font-semibold text-zinc-400 hover:text-zinc-700 px-1.5">{t("cancel")}</button>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2.5 bg-zinc-50/40">
              {threadLoading ? (
                ...
                <p className="text-[12px] text-zinc-300 text-center py-8">{t("emptyThread")}</p>
              ) : (
                thread.map((m, i) => {
                    ...
                    <div key={m.id} className={`flex flex-col ${m.from_admin ? "items-start" : "items-end"}`}>
                      {showLabel && (
                        <p className={`text-[10px] font-black uppercase tracking-wide mb-1 px-1 ${m.from_admin ? "text-zinc-400" : "text-zinc-500"}`}>
                          {m.from_admin ? t("maintlyTeam") : t("you")}
                        </p>
                      )}
